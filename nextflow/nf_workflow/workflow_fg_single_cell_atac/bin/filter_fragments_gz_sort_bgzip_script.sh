...
awk -v threshold="$threshold" -v FS='[,|\t]' 'NR==FNR && ($2-$3-$4-$5)>threshold {Arr[$1]++;next} Arr[$4] {print $0}' "$temp_summary_dict" <(zcat "$in_frag_tsv_gz") | sort -k1,1 -k2,2n -k3,3n -o "${in_frag_tsv_gz}.sorted" | bgzip -l 5 -@ 16 -c > "$output_file_name"
