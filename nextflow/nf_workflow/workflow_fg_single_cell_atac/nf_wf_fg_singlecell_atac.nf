...
workflow {
  ...
  files_ch = Channel
    ...
    .map { row -> tuple( file(row.R1_fastq_gz), file(row.R2_fastq_gz), file(row.R3_fastq_gz), file(row.R4_fastq_gz),file(row.barcode1_fastq_gz),file(row.barcode2_fastq_gz),file(row.spec), file(row.whitelist),row.subpool,file(row.barcode_conversion_dict_file)) }
    .set { sample_run_ch }
  ...
  // run_calculate_tss_enrichment(params.ATAC_TSS_CALCULATION_SCRIPT,fragments_filter_out,tbi_fragments_out,regions_ch,params.ATAC_TSS_BASES_FLANK,params.ATAC_TSS_COL_WITH_STRANDS_INFO,params.ATAC_TSS_SMOOTHING_WINDOW_SIZE)
