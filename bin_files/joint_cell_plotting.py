...
def parse_arguments():
    ...
    parser.add_argument("--pkr", type=str, help="PKR name", nargs='?', default="")
    parser.add_argument("--rna_metrics_file", type=str, help="Filename for RNA metrics tsv file")
    parser.add_argument("--atac_metrics_file", type=str, help="Filename for ATAC metrics tsv file")
    ...
    parser.add_argument("--barcode_metadata_file", type=str, help="Filename for barcode metadata csv file")
...
def get_metrics(rna_metrics_file, atac_metrics_file, remove_low_yielding_cells):
    ...
    for line in rna_metrics_contents:
        if int(float(line[1])) >= remove_low_yielding_cells:
            umis.append(int(float(line[1])))
    ...
    for line in atac_metrics_contents:
        if int(line[1])/2 >= remove_low_yielding_cells:
            tss.append(float(line[4]))
            frags.append(int(line[1])/2)
...
def main():
    ...
    metrics_df = get_metrics(args.rna_metrics_file, args.atac_metrics_file, args.remove_low_yielding_cells)

    # QC cells based on inputted cutoffs
    logging.info("QCing cells\n")
    metrics_df = qc_cells(metrics_df, args.min_umis, args.min_genes, args.min_tss, args.min_frags)

    # generate plot
    logging.info("Generating joint cell calling plot\n")
    plot_cells(metrics_df, args.pkr, args.min_umis, args.min_genes, args.min_tss, args.min_frags, args.plot_file)

    # save dataframe
    logging.info("Saving dataframe as csv\n")
    metrics_df.to_csv(args.barcode_metadata_file)
