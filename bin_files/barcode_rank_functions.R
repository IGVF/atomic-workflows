...
print ('&&&&&&&&&&&source barcode_rank_functions.R')

get_vectors <- function(x, y){
  print("Starting get_vectors method.")
  smooth_spline <- smooth.spline(x, y, spar=1)
  second_deriv <- predict(smooth_spline, x, deriv=2)
  
  # Second derivative values can be noisy at the beginning and end of the graph; exclude first 10% and last 10% 
...
# Takes in a set of xy coordinates of the plot and mode, returns the point which is farthest 
...
elbow_knee_finder <- function(x, y, mode="basic") {
  print("Starting elbow_knee_finder method.")
  # With advanced mode, use the helper function to determine which vectors to perform calculation on
  if (mode == "advanced") {
    # smooth.spline() function used in get_vectors() requires at least 4 unique
    # x values; preempt this error
    if (length(unique(x)) < 4) {
      print("Not enough unique x values. Returning NULL.")
      return(NULL)
    } else {
  ...
  # Error case: return NULL if vectors have a length of 0
  if (length(x)==0 | length(y)==0) {
    print("Input vectors have length 0. Returning NULL.")
    return(NULL)
  }
...
# before the first elbow/knee (i.e., the elbow/knee of all barcodes and the elbow/knee
...
get_elbow_knee_points <- function(x, y) {
  print("Starting get_elbow_knee_points method.")
